...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("nmap_scan.log"),  # Log to a file
...
async def run_nmap(target: str, options: str = "") -> None:
    """
    Runs an nmap scan asynchronously for the given target.
    Saves the output as an XML report in the reports folder.
    """
    ...
    report_filename = os.path.join(REPORT_DIR, f"nmap_scan_{unique_id}.txt")

    # Construct the safe command with -oX for XML output
    command = f"nmap -oX {shlex.quote(report_filename)} {options} {shlex.quote(target)}"
...
async def start_scan(request: ScanRequest):
    ...
    try:
        ...
        asyncio.create_task(run_nmap(target, options))  # Fire-and-forget
