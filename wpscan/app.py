...
from wpscan_tasks import run_wpscan_task
...
async def run_wpscan():
    """Handle WPScan scan requests asynchronously by offloading to Celery."""
    target = request.json.get('target')
    scan_type = request.json.get('scan_type', '--random-user-agent')  # WAF Bypass

    if not target:
        return jsonify({"error": "Target IP/Domain is required"}), 400

    # Offload the WPScan to Celery
    task = run_wpscan_task.delay(target, scan_type)
